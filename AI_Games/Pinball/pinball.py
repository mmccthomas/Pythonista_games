...
# test hole action
# TODO flipper interaction has problems
...
pinball_image = 'pinball1.png'
json_file = f'{pinball_image.split(".")[0]}.json'
...
class Wall:
    ...
    def __init__(self, name, centroid, coordinates, outside=True):
        self.name = name
...
class Flipper:
    ...
    def update(self):
        ...
        if True: #self.min_angle < self.angle < self.max_angle:
            self.node.rotation = np.radians(self.angle)
            position, _ = self.transform_perimeter(
                                        np.array(self.bbox.center()),
                                        np.array(self.bbox.center()), # rotation centre
                                        np.array(self.pivot), # stationary pivot point
                                        np.radians(self.angle))
            
            self.node.position = position
            self.transform_perimeter(self.coordinates, 
                                     np.array(self.bbox.center()), # rotation centre
                                     np.array(self.pivot), # stationary pivot point
                                     np.radians(self.angle))        
...
class Ball:
    ...
    def place_start(self, objects):
        ...
        #self.start_pos=np.array([700, 900])
        #self.vel= np.array([-14, -7])
    ...
    def circle_points(self, n=8):
        angles = np.linspace(0, 2 * np.pi, n, endpoint=False)
        x = self.pos.x + self.radius * np.cos(angles)
        y = self.pos.y + self.radius * np.sin(angles)
        return np.column_stack((x, y))
...
class Pinball(Scene):
    ...
    def draw_image(self):
        w, h = get_screen_size()
        img = Image.open(pinball_image)
        
        
        self.map_image = ui.Image.named(pinball_image)
    ...
    def update(self):
        ...
        min_y = [wall.bbox for wall in self.walls if not wall.outside][0][1] +10
    ...
    def deflect_ball(self, point, object, closest, normal):
        ...
        if True: #vel_along_normal < 0:  # only reflect if moving INTO the wall
    ...
    def collide_wall(self, wall):
        ...
        point = self.ball.touch_point()
        if self.hit_test(coords, point, inside=wall.outside):                 
    ...
    def collide_bumper(self, bumper):
        ...
        """  
        diff = self.ball.pos - b.pos
        if np.linalg.norm(diff) < self.ball.radius + b.radius:
            self.score += b.score  # Increase score on hit
            if np.linalg.norm(diff) == 0:
                mormal = np.array([0,0])
            else:
                normal = diff / np.linalg.norm(diff)
            self.ball.pos = b.pos + normal * (self.ball.radius + b.radius)
            self.ball.vel = (self.ball.vel - 2 * np.dot(
                self.ball.vel, normal) * normal) * b.bounce
        """          
