...
# choosing a smaller set gives better outcome
...
STATS = { 4: 4, 5: 10, 6: 10, 7: 7, 8: 5, 9: 2}
def add(a, b):
...
class Cross(KrossWord):
  ...
  def create_word_search(self, words, size=15):
      ...
      for word in words:
        w = word.replace(' ','')
        success, coord = word_square_gen.place_word(board,w,coords,max_iteration=500, space=' ')
        if success:
  ...
  def wordsearch(self, size=13, no_start=38, iterations=10):   
        ...
        # need stats to give existing puzzles names
  ...
  def fill_remaining(self):
      ...
      unfilled = self.board.size - np.count_nonzero(np.char.isalpha(self.board))
  ...
  def place(self, start_loc, dirn, possibles):
      ...
      word = Word(start_loc, self.dir_str[dirn].upper(), len(possibles), word=''.join(possibles), index=len(self.word_locations))
      ...
      if self.debug:
          print()
          caller =  str(inspect.currentframe().f_back).split(',')[-1].removeprefix(' code ')[:-1]
          print(f'placed  {word.word.capitalize()}@{word.start} from {caller}')          
      return True            
  ...
  def stats(self):
     ...
     t=time()
     self.frame_dict = {w: v for w, v in w_dict.items() if w.endswith('_frame:')}
     ...
     # count words in each puzzle, length of words and number in starting letters
     puzzles = []
     ...
     print()
     print('word lengths in words list', dict(sorted(Counter([len(words) for words in self.wordset if words]).items())))
     print('letter density in words list', dict(sorted(Counter([words[0] for words in self.wordset if words]).items())))
  ...
  def compute_puzzle_text(self, name='puzzle', add_coords=True):
        """produce all text """
        if add_coords:
            words = sorted([word.word.upper()+str(word.start)+word.direction.upper() for word  in self.word_locations])
        else: 
...
if __name__ == '__main__':
  ...
  print('elapsed', time()-t)
  #best, frame = cx.monte_carlo(size=13, no_start=34, start_iterations=10, iterations=10)
  text = cx.compute_puzzle_text(name=f'{cx.base}{cx.next_number}', add_coords=False)
  print(text)
  if save:
