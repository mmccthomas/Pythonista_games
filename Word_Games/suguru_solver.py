...
from itertools import permutations, cycle, count
...
class Cell():
    ...
    def get_neighbour_vals(self, board,  N=3):
        ...
        self.neighbour_vals = board[max(self.r - (N-2), 0):min(self.r + N-1, board.shape[0]),
    ...
    def get_neighbours(self, board,  N=3):
        ...
        self.neighbours = [(r1, c1) 
                           for r1 in range(top, bottom)
...
class Board():
    ...
    def get_possibles(self): 
        ...
        return '\n'.join([' '.join([f'{member.coordinate}, {member.possibles}' 
                                    for member in cage.members])
                          for cage in self.cages])
                          
    def flat_list(self):
        return sorted(sum([cage.coordinates() for cage in self.cages], []), key=itemgetter(0,1))
...
class SuguruSolve(): 
  ...
  def filter_same_possibilities(self):
    """if 2 cells  has  samepossibilities,  and more than 1 adjacent cells have
    same possibilities, then adjacent cell cannot have those possiniles
    if cell has [A, B, C] and neighbours have [A, B] or [B, A], cell cannot have [A, B]"""
    for cage in self.board_obj.cages:
       for cell in cage.members:
          base_poss = cell.possibles
          if len(base_poss) < 2:
              continue
          neighbours = [self.board_obj.find_cell(loc) 
  ...
  def filter_larger(self):
    """if a cell is adjacent to all cells in a neighbouring group, it has to have values higher than length of neighbouring group"""
    for cage in self.board_obj.cages:
       for cell in cage.members:
          base_poss = cell.possibles
          neighbours = [self.board_obj.find_cell(loc) 
          ...
          parents = [neighbour.parent(self.board_obj) for neighbour in neighbours]
          if len(set(parents)) == 1 and len(parents) == parents[0].length:
              if cell.parent(self.board_obj).length > parents[0].length:
                [cell.remove_possible(p) for p in range(1, parents[0].length+1)]
                print(f'removed {list(range(1, parents[0].length+1))} from {cell}')
   
  def choose(self):
    """Typically, the logic is along the lines of "if this cell is a 5, then this pair of cells could be 3/7 or 7/3 with no way of determining which. Since the puzzle has a unique solution, the first cell must not be a 5"."""
  ...
  def compute_known(self, N, method=1):
    ...
    if method == 0: # 
      ...
      known = []      
      cages = sorted(self.board_obj.cages, key=attrgetter('length'), reverse=True)
      non_unity = [cage.index for cage in cages if cage.length >1]
      indices = []
      i=0
      while len(indices) < N:    
        idx = non_unity[i % len(non_unity)]
  ...
  def fill_remaining(self):
    """ starting with partially filled board, fill the rest with depth first search """
    self.permutation_dict = {k: list(permutations(list(range(1, k + 1))))
                                 for k in range(1, 7)}
    self.fill_iteration = 0
    self.initial_depth = len(inspect.stack())
    self.empty_cells = [cell for cage in self.board_obj.cages for cell in cage.members  if cell.value == 0]
    while not np.array_equal(self.board_obj.board, self.sol_obj.board):
        # print(f'{self.board_obj.board=}\n{self.sol_obj.board=}')
        [cell.update_board(self.board_obj.board, 0) for cell in self.empty_cells]
  ...
  def run(self):
      ...
      counts={}
      visible ={'5x5':  0, '6x6': 6, '7x7': 6, '8x8': 20, '9x9': 25}             
      ...
      for group, items in self.puzzles_from_file.items():
        ...
        for solution, cages in items:
            ...
            self.board_obj = Board(cages) #deepcopy(self.sol_obj)
            self.board_obj.board = np.zeros(self.sol_obj.board.shape, dtype=int)
            ...
            while not np.array_equal(self.board_obj.board, self.sol_obj.board):
              ...
              while np.any(self.board_obj.board==0):
                  ...
                  # print(self.board_obj.print_cage_board(which=f'Board{i}_{group},start {i1} iteration {index}',highlight=items))    
              ...
              if i1>500:
                  break               
              #print('#'*32)
            board_solved = np.array_equal(self.board_obj.board, self.sol_obj.board)
            print([c.coordinate for c in known])
            counts[group].append(board_solved)
            print(f'Board {i} in {group} solved {board_solved} in {(time()-t):.1f}s {i1} start points{"*"*1}')
            print(self.board_obj.print_cage_board(which=str(i),highlight=items))
            i += 1
