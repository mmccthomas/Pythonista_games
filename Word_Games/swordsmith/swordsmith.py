...
#WORDLIST = '../wordlists/5000-more-common.txt'
...
class AmericanCrossword(Crossword):
    ...
    def from_grid(cls, grid):
        """Generates American Crossword from 2D array of characters"""
        rows = len(grid)
        cols = len(grid[0])
...
class BritishCrossword(Crossword):
    """ this style has many less intersections"""
   
    def __init__(self, rows, cols):
        super(BritishCrossword, self).__init__()
    ...
    @classmethod
    def from_grid(cls, grid):
        """Generates British Crossword from 2D array of characters"""
        rows = len(grid)
        cols = len(grid[0])
...
def run_test(**args):
    ...
    wordlist = read_wordlist(WORDLIST, scored=False)
    ...
    for _ in range(args.num_trials):
        ...
        crossword = BritishCrossword.from_grid(grid)
...
def main():
    ...
    parser.add_argument('-g', '--grid', dest='grid_path', type=str,
                        default='13xcmt.txt', help='filepath for grid')
    ...
    """
    run_test(wordlist_path='spreadthewordlist.dict', grid_path='13xcmt.txt', num_trials=1, animate=True,strategy='dfs',k=5)
