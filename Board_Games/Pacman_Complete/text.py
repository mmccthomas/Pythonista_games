...
class Text(object):
    ...
    def render(self, screen, update=False):
        if self.visible:
            ...
            x, y = self.position                
            y = NROWS * TILEHEIGHT - y -50 # GRID_POS[1] - y
            x = GRID_POS[0] + x
            if update:
                ...
                LabelNode(self.text, position=(x, y), color=self.color, font=('Marker Felt', self.size), parent=screen)
...
class TextGroup(object):
    ...
    def render(self, screen, update=False):
        for tkey in list(self.alltext.keys()):
            self.alltext[tkey].render(screen, update=update)
