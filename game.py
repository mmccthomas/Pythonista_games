...
from objects import *
from common import xsize, ysize
import random
from common import KyeFile
...
class KGame:
    ...
    def __init__(self, f, want_level, movesource):
      
        kyefile = KyeFile(f)
        grid = kyefile.get_grid(want_level)
        
        self.levels = kyefile.get_no_levels()         
        self.thislev = kyefile.current_level
        self.levelnum = kyefile.get_level_number()
        
        self.hint = kyefile.hint
        self.exitmsg = kyefile.exitmsg
        self.nextlevel = kyefile.get_next_level()
        
        self.board = [None] * (xsize*ysize)
        self.invalidate = [1] * (xsize*ysize)
        self.magnet_count = [0] * (xsize*ysize)
        ...
        for y in range(ysize):
            for x in range(xsize):
                ...
                try:
                    ...
                    # call class with variable number of parameters
                    cc = ctype[0](*ctype[1:])
                    # populate board
                    self.add_at(x, y, cc)
        ...
        for r in  range(40):
            ...
            for t in range(4*r+1):
                if r == 0:
                    ...
                    x = self.kyestart[0] + random.randint(-r, r)
                    y = self.kyestart[1] + random.randint(-r, r)
                if self.get_atB(x, y) == None:
                    self.add_at(x, y, k)
                    return k

        # Hmm, this shouldn't happen but, if we do fail to find a square...
        raise KyeGameRuntimeError('failed to find a space for kye')
    ...
    def dotick(self):
        ...
        for f, t in current_thinkers:
            if tics % f == 0:
                ...
                except (KeyError) as e: # Object was deleted during this tick
                    continue
...
class KyeGameRuntimeError(BaseException):
    pass

class KGameFormatError(BaseException):
    pass
