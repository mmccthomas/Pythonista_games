...
class TileGame(Scene):
	...
	def setup_ui(self, first_time=True):
		...
		if TILES == "pictures":
			if not image_file:
				...
				selection = random.randint(0,len(all_assets))
	...
	def step_through(self, path):
		...
		for node in path:
			...
			tile_no =node		
			#logging.info(f'tile {tile_no} moved from {(r_from,c_from)} to {(r_to,c_to)}')
			...
			#for i in range(SIZE):
			#	logger.info(new_arr[i, :]) 
			#logger.info( "")
			init_array = new_arr
			self.no_moves += 1
		# logger.info(str(self.move_array))	
		self.paused = False
		self.update_score()
		return
