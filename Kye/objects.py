...
"""Classes for individual objects in the game,
 implementing their behaviour and animations."""
...
class KyeGhost(Thinker):
    """This is the ghost of a dead kye. It lasts just a few frames
     and them removes itself."""
    ...
    def think(self, game, x, y):
        ...
        if self.frame > 5:  # made a bit longer
            if self.kye.lives >= 0:
...
class Block(Thinker):
    ...
    def __init__(self, t, round, timer=0):
        ...
        timer -- normally omitted, but if positive then it makes a timer block
        with the number indicating how long until it expires.
        """
        Thinker.__init__(self)
        self.timer = 0
    ...
    def turn(self):
        """Returns -1 or 1 if sliders/rounders hitting this block
        should be turned left or right; 0 for an ordinary block."""
        return self.__turn

    def image(self, af):
        if self.timer > 0:
          return "block_timer_"+str(self.timer//30)
        if self.round:
          return "blockr"
        if self.__turn == -1: 
...
class Shooter(Thinker):
    ...
    def think(self, game, x, y):
        ...
        if self.__waiting > y and b is not None:
...
class BlackHole(Thinker):
    ...
    def swallow(self, g, animate=True):
        """Called whenever something might fall in. Returns true if we swallow it,
        false if we cannot.
...
class OneWay(Base):
    ...
    def allow_move(self, dx, dy):
        """This checks a possible move onto the black hole and returns true if
        it matches the door's allowed direction."""
