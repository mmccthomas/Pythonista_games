...
from objects import *
from common_cmt import xsize, ysize
...
class KGame:
        ...
        for y in range(ysize):
            ...
            for x in range(xsize):
                ...
                try:
                    ...
                    print(e, f"Unable to load {c} at {x},{y}")

        # If no  in the level, add it at 3,3, as Kye 2.0 does
    ...
    def get_at(self, i, j):
        """Return the content of tile (i, j). 
        Caller must ensure that (i, j) is inside the game board."""
        return self.board[xsize*j + i]

    def get_atB(self, i, j):
        """Returns the content of tile (i, j), or a Wall if (i, j) is
    ...
    def magnet_range(self, x, y, d):
        """Update the magnet effect table to allow for the addition 
        (if d=1) or removal (if d=-1) of a magnet at (x, y)."""
        pos = xsize*y + x
        maxp = xsize*ysize
    ...
    def push_object(self, x, y, dx, dy):
        """The object at (x, y) is pushed by another object in direction (dx, dy)
         (which should be +/-1 and not both 0) - work out if it can move and what happens to it."""
        ...
        if dx != 0 and dy != 0:
            if self.get_atB(x, ty) != None or self.get_atB(tx, y) is not None:
                return False
...
# are attempting accordingly.
    ...
    def dotick(self):
        """Run one game tick - move the Kye and animate/move all active objects in 
        the game."""
