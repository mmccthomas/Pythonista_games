...
class Adventure():
  ...
  def load_location_map(self):
      ...
      Note: locations were identified using Gemini AI on image """
  ...
  def get_walkthru(self, filename):
      ...
      with open(filename, 'r') as f:
          d = f.read()
      commandlist = self.extract_between(d)
  ...
  def replace_statements(commandlist, to_replace):
      """ replace a list of (from, to) replacements in commandlist """
  ...
  def remove_statements(commandlist, to_remove):
      """ remove items starting with list of characters from commandlist """
  ...
  def get_vocabulary(self, _print=False):
      """ get all vocabulary into dictionary of lists """
